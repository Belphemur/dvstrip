...
3. **No feedback loops:** `isOwnOutput()` filters the `.dvstrip.tmp` suffix and the `.hdr10`-style output suffixes from scan/watch inputs; the tmp name also never matches the video extension allow-list.
