...
ffmpeg never writes onto the input. Output goes to `<name>.dvstrip.tmp.mkv` **next to the source**, gets **verified** by a fresh probe (parses, same width, DV gone, marker present), and only then `os.Rename` publishes it — atomic on POSIX. A failed/killed run leaves the original untouched. **`--replace` keeps no backup.** `--dry-run` overrides everything.
